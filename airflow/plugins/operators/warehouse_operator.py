...
from helpers.staging_queries import create_staging_schema, drop_staging_tables, create_staging_tables, \
    copy_staging_tables
...
class WarehouseOp(BaseOperator):
    ...
    def execute(self, context):
        ...
        self.log.info("Creating Staging Schema.")
        ...
        self.log.info("Loading Staging Tables.")
        self.execute_query(conn, cur, copy_staging_tables)

        self.log.info("Creating Warehouse Schema.")
        self.execute_query(conn, cur, [create_warehouse_schema])

        self.log.info("Creating Warehouse Tables.")
        self.execute_query(conn, cur, create_warehouse_tables)

        self.log.info("Running Upsert Queries.")
